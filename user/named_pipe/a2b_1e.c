...
int main(int argc, char *argv[]){                                  
    int fd_r = open("a2b.pipe",O_RDONLY);
    ERROR_CHECK(fd_r,-1,"open a2b pipe");
    int fd_w = open("b2a.pipe",O_WRONLY);
    ERROR_CHECK(fd_w,-1,"open b2a pipe");
    ...
    while(1){
        ...
        if(FD_ISSET(fd_r,&set)){
            ...
            if(read_fd>0){
                ...
                /* fflush(stdout); */
                printf("\n");
                if(strcmp(buf,"1")==0&&buf[0]=='1'&&strlen(buf)==1){
                    printf("user exit\n");
                    break;
                }
            }else if(read_fd==0){
